...
import { PlusCircle, ArrowLeft, Loader2, Edit, Trash2, XCircle, File as FileIcon } from 'lucide-react';
...
const UnitForm = ({ unit, onSave, onCancel }: { unit?: Unit | null, onSave: () => void, onCancel: () => void }) => {
    ...
    const [uploading, setUploading] = useState<{ lang: 'EN' | 'SI', index: number } | null>(null);

    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, lang: 'EN' | 'SI', index: number) => {
        if (!storage) return;
        const file = e.target.files?.[0];
        if (!file) return;

        setUploading({ lang, index });
        const filePath = `units/${formData.unitNo}_${lang}_${file.name}`;
        ...
        try {
            ...
            const updatedPdfs = [...(formData[lang === 'EN' ? 'pdfsEN' : 'pdfsSI'] || [])];
            updatedPdfs[index] = { ...updatedPdfs[index], fileName: file.name, downloadUrl: downloadUrl };

            setFormData(prev => ({
                ...prev,
                [lang === 'EN' ? 'pdfsEN' : 'pdfsSI']: updatedPdfs
            }));
    ...
    const addPdfField = (lang: 'EN' | 'SI') => {
        const key = lang === 'EN' ? 'pdfsEN' : 'pdfsSI';
        const newPdf = { partName: '', fileName: '', downloadUrl: '' };
        setFormData(prev => ({
            ...prev,
            [key]: [...(prev[key] || []), newPdf]
        }));
    ...
    const removePdfField = (lang: 'EN' | 'SI', index: number) => {
        const key = lang === 'EN' ? 'pdfsEN' : 'pdfsSI';
        const updatedPdfs = [...(formData[key] || [])];
        updatedPdfs.splice(index, 1);
        setFormData(prev => ({ ...prev, [key]: updatedPdfs }));
    };

    const handlePdfPartNameChange = (lang: 'EN' | 'SI', index: number, value: string) => {
        const key = lang === 'EN' ? 'pdfsEN' : 'pdfsSI';
        const updatedPdfs = [...(formData[key] || [])];
        updatedPdfs[index] = { ...updatedPdfs[index], partName: value };
        setFormData(prev => ({ ...prev, [key]: updatedPdfs }));
    };
    ...
    const renderPdfFields = (lang: 'EN' | 'SI') => (
        <div className="space-y-4">
            <h4 className="font-semibold">{lang === 'EN' ? 'English' : 'Sinhala'} PDFs</h4>
            {(formData[lang === 'EN' ? 'pdfsEN' : 'pdfsSI'] || []).map((pdf, index) => (
                <div key={index} className="flex items-end gap-2 p-2 border rounded-md">
                    <div className="flex-grow space-y-2">
                        <Input
                            placeholder="Part Name (e.g., Note, Assignment Part 1)"
                            value={pdf.partName}
                            onChange={(e) => handlePdfPartNameChange(lang, index, e.target.value)}
                        />
                         <div className="flex items-center gap-2">
                             <Input type="file" onChange={(e) => handleFileChange(e, lang, index)} className="flex-grow" />
                             {uploading?.lang === lang && uploading?.index === index && <Loader2 className="w-4 h-4 animate-spin"/>}
                         </div>
                        {pdf.fileName && <p className="text-xs text-muted-foreground flex items-center gap-1"><FileIcon className="w-3 h-3"/> {pdf.fileName}</p>}
                    </div>
                    <Button type="button" variant="destructive" size="icon" onClick={() => removePdfField(lang, index)}><XCircle className="w-4 h-4" /></Button>
                </div>
            ))}
            <Button type="button" variant="outline" size="sm" onClick={() => addPdfField(lang)}>Add {lang} PDF</Button>
        </div>
    );
        ...
        <Card>
            <form onSubmit={handleSubmit}>
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div><Label>Price Notes (SI)</Label><Input value={formData.priceNotesSI} onChange={e => setFormData({...formData, priceNotesSI: e.target.value})} /></div>
                        <div><Label>Price Assignments (SI)</Label><Input value={formData.priceAssignmentsSI} onChange={e => setFormData({...formData, priceAssignmentsSI: e.target.value})} /></div>
                        <div><Label>Price Notes (EN)</Label><Input value={formData.priceNotesEN} onChange={e => setFormData({...formData, priceNotesEN: e.target.value})} /></div>
                        <div><Label>Price Assignments (EN)</Label><Input value={formData.priceAssignmentsEN} onChange={e => setFormData({...formData, priceAssignmentsEN: e.target.value})} /></div>
                    </div>
                                            ...
                                            <CardHeader className="flex flex-row justify-between items-start">
                                                ...
                                                <div className="flex gap-2">
                                                    ...
                                                    <AlertDialog>
                                                        ...
                                                        <AlertDialogContent>
                                                            <AlertDialogHeader><AlertDialogTitle>Confirm Deletion</AlertDialogTitle><AlertDialogDescription>Are you sure you want to delete this unit? This will remove all its data and associated PDFs. This action cannot be undone.</AlertDialogDescription></AlertDialogHeader>
                                                            <AlertDialogFooter>
